...
const TestimonialCard = ({ name, role, rating, text, image, companyLogo, onClick }) => {
  const stars = Array(5).fill(0).map((_, index) => (
    <span key={index} className={`text-2xl ${index < rating ? 'text-orange-500' : 'text-gray-300'}`}>
      ★
    </span>
  ))
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <img src={companyLogo} alt="Company Logo" className="h-4 w-auto object-contain" />
          </div>
        </div>
      </div>
      <div className="flex mb-4">{stars}</div>
      <p className="text-gray-700 mb-4 line-clamp-3">{text}</p>
      <button className="text-[#947335] hover:underline">Read more</button>
    </motion.div>
  )
}
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-300">•</span>
                  <img src={testimonial.companyLogo} alt="Company Logo" className="h-5 w-auto object-contain" />
                </div>
              </div>
            </div>
            <div className="flex mb-6">
              {Array(5).fill(0).map((_, index) => (
                <span key={index} className={`text-3xl ${index < testimonial.rating ? 'text-orange-500' : 'text-gray-300'}`}>
                  ★
                </span>
              ))}
            </div>
